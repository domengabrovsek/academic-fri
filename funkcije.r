...
{
  # odstranimo negativne vrednosti
  index <- which(data[, "PM10"] < 0)
  data <- data[-index, ]

  # disktretizacija PM10
  classes <- cut(data$PM10, c(0, 35, Inf), c("NIZKA", "VISOKA"))
  data[, "PM10"] <- classes
...
# priprava atributov za O3
PrepareAttributesO3 <- function(data)
{
