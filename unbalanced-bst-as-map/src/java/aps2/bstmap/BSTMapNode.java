...
public class BSTMapNode {
	...
	public boolean add(BSTMapNode element) {
		...
		else if(this.getKey() > element.getKey())
		{
			if(left == null)
			{
				left = new BSTMapNode(null, null, this, element.getKey(), element.value);
				return true;
			}
			else
			{
			...
			if(right == null){
				right = new BSTMapNode(null, null, this, element.getKey(), element.value);
				return true;
			} else
	...
	public String get(BSTMapNode element) {
		...
		{
			if (left != null) return left.get(element);
		}
		else if(this.getKey() < element.getKey())
		{
			if (right != null) return right.get(element);
		}
		else{
			return null;
		}
